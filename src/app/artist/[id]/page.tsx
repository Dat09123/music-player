...
export async function generateMetadata({ params }: Props) {
  ...
  try {
    ...
    return {
      title: `${artist.name} - Spotify Music`,
      description: `${artist.name} artist page - top tracks, albums, and related artists`,
    }
  } catch {
    return { title: "Artist - Spotify Music" }
  }
}
...
export default async function ArtistPage({ params }: Props) {
  ...
  try {
    ...
    const related = await getRelatedArtists(id)
    ...
    return (
      <div>
        ...
        <div className="relative overflow-hidden">
          {/* Background gradient */}
          <div
            className="absolute inset-0 bg-gradient-to-b from-transparent via-zinc-900/80 to-zinc-900 z-10"
          />
          {artist.images?.[0]?.url && (
            <div
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
        ...
        <p className="text-zinc-400">This artist could not be loaded. Please check the ID and try again.</p>
      </div>
    )
  }
}
